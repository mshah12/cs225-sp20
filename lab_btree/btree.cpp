...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    bool empty = subroot->elements.empty();
    if(empty == false) {
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    new_right->elements.vector::assign(mid_elem_itr + 1, child->elements.end());
    new_left->elements.vector::assign(child->elements.begin(), mid_elem_itr);
    new_right->children.vector::assign(mid_child_itr, child->children.end());
    new_left->children.vector::assign(child->children.begin(), mid_child_itr);
}
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    bool flag = false;
    bool empty = subroot->elements.empty();
    if(!empty && subroot->elements.size() > first_larger_idx && pair == subroot->elements[first_larger_idx]) {
      return;
    }
    if(subroot->is_leaf) {
    ...
    if(newChild->elements.size() < order) {
      flag = true;
    }
    if(flag) {
      return;
