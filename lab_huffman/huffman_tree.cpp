...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     for(unsigned long i = 0; i < frequencies.size(); i++) {
       TreeNode* tempNode = new TreeNode(frequencies[i]);
       singleQueue.push(tempNode);
     }
     while(singleQueue.size() + mergeQueue.size() > 1) {
       ...
       int newFrequency = temp1->freq.getFrequency() + temp2->freq.getFrequency();
       TreeNode* newNode = new TreeNode(newFrequency);
       newNode->left = temp1;
       newNode->right = temp2;
     ...
     */
     while(bfile.hasBits() == true) {
       if(bfile.getNextBit() == 0) {
         ...
         tempNode->left = readTree(bfile);
         tempNode->right = readTree(bfile);
         return tempNode;
       }
       else {
         TreeNode* tempNode2 = new TreeNode(Frequency(bfile.getNextByte(), 0));
         return tempNode2;
       }
     }
     return NULL;
