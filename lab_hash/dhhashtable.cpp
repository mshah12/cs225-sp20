...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
     ...
     */
     ...
     if (shouldResize())
      ...
      size_t idx1 = hashes::hash(key, size);
      size_t idx2 = hashes::secondary_hash(key, size);
      size_t end = idx1;
      while(should_probe[idx1] == true) {
        count++;
        idx1 = (end + (idx2 * count)) % size;
      }
      if(table[idx1] != NULL) {
        delete table[idx1];
        table[idx1] = NULL;
        table[idx1] = temp;
        should_probe[idx1] = true;
        return;
      }
      table[idx1] = temp;
      should_probe[idx1] = true;
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
    ...
    size_t idx2 = hashes::secondary_hash(key, size);
    size_t end = idx1;
    if(!should_probe[idx1]) {
      return -1;
    }
    if(table[idx1] != NULL && table[idx1]->first == key) {
      return idx1;
    }
    idx1 = (idx1 + idx2) % size;
    while(idx1 != end) {
