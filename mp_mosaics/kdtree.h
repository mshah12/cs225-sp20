...
class KDTree
{
  ...
  public:
    ...
    KDTreeNode* KDTreeHelper(int startIndex, int endIndex, int curDim);
    ...
    KDTreeNode* findNearestNeighborHelper(const Point<Dim>& query, typename KDTree<Dim>::KDTreeNode* subroot, int curDim) const;
  ...
  private:
     ...
     */
    int quickSelect(int left, int right, int pivotPoint, int curDim);

    int partition(int left, int right, int curDim);

    double calculateDist(const Point<Dim> p1, const Point<Dim> p2) const;

    void destroyFunction(KDTreeNode* subRoot);

    void copyFunction(const KDTree<Dim>& other);
};
